...
(function() {
	var undefined;
	/**
	@name woosh.ResultSet
	@constructor
	@description A collection of Result objects allowing serializing / unserializing
	
	@param {string} [name] A name for this set
	*/
	function ResultSet(name) {
		this.name = name;
		this.results = {};
	}
	
	woosh._utils.extend(ResultSet, Object, {
		/**
		@name woosh.ResultSet#results
		@type {object}
		@description An object of test results (keyed by either test name or library name, depending on the instance)
		*/
		results: {},
		/**
		@name woosh.ResultSet#name
		@type {string}
		@description Name of this set (usually test name or library name)
		*/
		name: undefined,
		/**
		@name woosh.ResultSet#serialize
		@function
		...
		*/
		...
		/**
		@name woosh.ResultSet#unserialize
		@function
		@description Populate object using a string generated by {@link woosh.ResultSet#serialize}
		...
		*/
		unserialize: function(serial) {
			...
			pos = serial.indexOf('#');
			if (i == -1) {
				throw new Errow('woosh.ResultSet#unserialize: Invalid serial');
			}
			// set name
...
(function() {
	...
	*/
	...
	if (window.name) {
		try {
			savedResultSet = new woosh.ResultSet().unserialize(window.name);
	...
	@param {woosh.ResultSet} resultSet Results to save 
	*/
...
(function() {
	...
	*/
	function LibraryTest(libraryName, tests) {
		this.tests = {};
		this.testNames = [];
		this.resultSet = new woosh.ResultSet(libraryName);
	...
	LibraryTest.prototype = {
		...
		*/
		testNames: [],
		/**
		@name woosh._LibraryTest#resultSet
		@type {woosh.ResultSet}
		...
		*/
		run: function(testName, onTestComplete) {
			...
			test._run(function(result) {
				...
				// add to resultset
				libraryTest.resultSet.results[testName] = result;
...
(function() {
	...
	@param {woosh.ResultSet[]} resultSets Array of result sets (where their name is the library name) to include in the runner.
	...
	*/
	...
	woosh._utils.extend(TestSetRunner, Object, {
		...
		*/
		highestIsBest: null,
		/**
		@name woosh._TestSetRunner#lastResults
		@type {object}
		...
		*/
		run: function(testName, onTestComplete, onComplete) {
			...
			function testComplete(testName, result) {
				if (result) {
					results.push(result);
					testSetRunner.lastResults[currentLibName] = result;
				}
				onTestComplete.call(testSetRunner, currentLibName, result);
				setTimeout(function() {
					runNextTest();
				}, 300);
			}
			...
			function runNextTest() {
				if (resultSetsIndex < resultSetsLen) {
					...
					testComplete( testName, resultSet.results[testName] );
					resultSetsIndex++;
				} else {
					...
					if (currentLibraryTest) {
						...
						testSetRunner._analyseResults(results);
	...
	*/
		...
		function testFrameReady() {
			if ( !--numOfFramesWaiting ) {
				...
				for (var i = 0, len = libraryNames.length; i<len; i++) {
					...
					conductor._resultSets[ libraryNames[i] ] = testFrame.libraryTest.resultSet;
				}
	...
	Conductor.prototype = {
		...
		*/
		...
		@type {Object}
		@description woosh.ResultSet objects keyed by library name
		*/
		...
		*/
		...
		@description Add an object to listen for conductor events. Conductor fires the following:
			...
			listener.allTestsComplete(resultSets)
				...
				- resultSets is an object of woosh.ResultSet keyed by library name
	...
	*/
	...
	Table.prototype = {
		...
		*/
		...
		@type {Object}
		@description woosh.ResultSet objects keyed on library name
		*/
		_resultSets: {},
		/**
		...
		*/
		_checkResults: function(testName, testSetRunner) {
			var resultRow = this._testRows[testName],
				...
				infoNode,
				warningMsg = '',
				results = testSetRunner.lastResults,
				result,
				testCell,
				resultPercent,
				cellBg,
			...
			for (var libraryName in results) {
				...
				if (resultDifference === 0) {
					...
					resultPercent = ( (result.result - testSetRunner.minResult) / (testSetRunner.maxResult - testSetRunner.minResult) );
				}
				...
				if (result.result === testSetRunner.maxResult || result.result === testSetRunner.minResult) {
